...
class FlowerListViewController: UITableViewController {
    ...
    private func goToFlower(_ flower: Flower) {
        self.performSegue(withIdentifier: "ToFlower", sender: flower)
    }
...
extension FlowerListViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        guard let cell = tableView.dequeueReusableCell(withIdentifier: "FlowerCell", for: indexPath) as? FlowerCell
        else {
            return FlowerCell()
        }
...
extension FlowerListViewController {
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let row = (indexPath as NSIndexPath).row
        guard row >= 0 && row < flowers.count else {
            return
        }
